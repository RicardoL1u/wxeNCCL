...
function show_help() {
    ...
    echo "Commands:"
    echo "  gen-deepcopy    generate deepcopy code"
    echo "  gen-client      generate clientset \ lister \ informer code"
    echo "  gen-crd         generate CRD yaml"
    # 添加其他命令的帮助信息
}

OUTPUT_BASE=/Users/claire/mizar/worksapce
# 设置软链或者目录结构 $OUTPUT_BASE/gitlab.infini-ai.com/mizar/infiniai-controller到当前目录
function gen_deepcopy() {
    kube::codegen::gen_helpers \
        --input-pkg-root gitlab.infini-ai.com/mizar/asterism/fault-tolerance/pkg/apis \
...
function gen_client() {
    kube::codegen::gen_client \
        --input-pkg-root gitlab.infini-ai.com/mizar/asterism/fault-tolerance/pkg/apis \
        --output-base $OUTPUT_BASE \
        --output-pkg-root gitlab.infini-ai.com/mizar/asterism/fault-tolerance/generated \
        --with-watch
}
...
function gen_crd() {
    ...
    gobin="${GOBIN:-$(go env GOPATH)/bin}"
    ${gobin}/controller-gen crd:generateEmbeddedObjectMeta=true paths=./pkg/apis/... output:crd:dir=./pkg/generated/manifests
}
